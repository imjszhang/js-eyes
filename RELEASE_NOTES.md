...
> `2.8.3`; bundled sub-skills ship independent versions (notably
> `js-x-ops-skill@3.5.0`, `js-hn-ops-skill@1.0.0`).

### Highlights

- **GitHub Pages CI**: push to `main` → `npm run build:site` → deploy `dist/`.
  Local preview: `npm run preview` (serve `dist/` on port 3000).
- **New bundled skill**: `js-hn-ops-skill` — Hacker News read / search /
  navigation (`hn_get_front_page`, `hn_get_item`, `hn_search`, …).
- **js-x-ops-skill 3.5.0**: official X API v2 CLI (`api status|timeline|tweets|
  mentions|trends|delete|tweet|…`), pinned-tweet support, post `budgetMs`.
- **js-browser-ops-skill 2.5.1**: `autoAllowDomain` on `browser_read_page` for
  smoother egress when reading new hosts.
- **Local server scripts**: `bin/js-eyes-server-start.sh` / `.cmd` — no `npx`.
- **Platform bump**: CLI、扩展、OpenClaw plugin、`@js-eyes/*` 核心包同步至 `2.8.3`。

### Migration Notes

- If you bookmarked `./docs/skills.json`, switch to the built registry at
  `dist/skills.json` locally or `https://js-eyes.com/skills.json` in production
  (generated by `npm run build:site`).
- Sub-skill upgrades: `js-eyes skills update --all` or
  `curl -fsSL https://js-eyes.com/install.sh | JS_EYES_SKILL=all bash`.
- `@js-eyes/visual-bridge-kit` / `visual-replay-hyperframes` are **not** tied
  to the platform semver — check their own package versions.

### Downloads

- [npm CLI (`js-eyes`)](https://www.npmjs.com/package/js-eyes)
- [npm scope (`@js-eyes/*`)](https://www.npmjs.com/org/js-eyes)
- [Chrome Extension](https://github.com/imjszhang/js-eyes/releases/download/v2.8.3/js-eyes-chrome-v2.8.3.zip)
- [Firefox Extension](https://github.com/imjszhang/js-eyes/releases/download/v2.8.3/js-eyes-firefox-v2.8.3.xpi)
- [Skill Bundle](https://github.com/imjszhang/js-eyes/releases/download/v2.8.3/js-eyes-skill-v2.8.3.zip)
- [ClawHub Skill (`js-eyes@2.8.3`)](https://clawhub.ai/imjszhang/js-eyes)

### Installation Instructions

1. Upgrade the `js-eyes` bundle to `2.8.3` and run `npm install` in the bundle
   root.
2. Reload browser extensions to `2.8.3` if you rely on platform-side fixes.
3. Run `js-eyes skills update --all` to pull sub-skill updates (x-ops, hn, …).
4. Restart OpenClaw if the plugin is loaded so router + skill bindings reload.
...
> and every public API are byte-for-byte compatible with 2.6.3 — modulo the
> additive `capture_screenshot` automation message — so `js-eyes skills update
...
- **`capture_screenshot` extension RPC** *(2026-05-02)*: Chrome 2.7.0
  ...
  a base64 PNG of the active tab — or `{ skipped: 'tab_not_active' }` when
...
- **`@js-eyes/visual-replay-hyperframes@0.2.0`** translator now emits a
  ...
  event's `viewport` (1280 × 720 fallback). Redact rects from `meta.json`
...
- **First Firefox 2.7.0 regression baseline** lives at
  ...
  (real-tab capture against `github.com/imjszhang/hyperframes` — see the
...
1. Download `js-eyes-chrome-v2.7.0.zip`, extract, reload unpacked (or
   ...
   already cover `chrome.tabs.captureVisibleTab` — no permission prompt
   delta vs 2.6.3.
...
1. Install `js-eyes-firefox-v2.7.0.xpi` (or update from AMO once the
   ...
   `browser.tabs.captureVisibleTab` — no permission delta vs 2.6.3.
...
>   unchanged — Firefox / Chrome `--visual` sessions still paint live
>   feedback.
> - New baseline:
