...
- The upgrade itself still requires one OpenClaw restart (so the new plugin code — `SkillRegistry` + watcher — is picked up). After that, further skill changes stay zero-restart.
