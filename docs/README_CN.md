...
| [OpenClaw](https://openclaw.ai/) + [openclaw-plugin](../openclaw-plugin) | 注册为 OpenClaw 插件 — 1 个 `js-eyes` 工具、后台服务、CLI 命令 |
...
1. 从 [js-eyes.com](https://js-eyes.com/js-eyes-skill.zip) 下载 `js-eyes-skill.zip`，或者从 [GitHub Releases](https://github.com/imjszhang/js-eyes/releases/latest) 下载带版本号的 `js-eyes-skill-v<version>.zip`（例如 `js-eyes-skill-v2.6.3.zip`）
...
JS Eyes 注册为 [OpenClaw](https://openclaw.ai/) 插件，只暴露一个 `js-eyes` 工具，并通过路径式 action 路由浏览器自动化与扩展技能能力。
...
- **1 个 AI 工具** — `js-eyes`，通过路径式 action 调用浏览器自动化与技能管理能力
...
| `browser/execute-script` | 在标签页中执行 JavaScript 并返回结果 |
| `browser/get-cookies` | 获取标签页对应域名的所有 Cookie |
| `skills/discover` | 查询技能注册表，列出可安装的扩展技能 |
| `skills/plan-install` | 下载、校验并生成扩展技能安装计划 |
...
5. 重启或刷新 OpenClaw — 服务器自动启动，AI Agent 可通过 `js-eyes` 工具控制浏览器。
...
- 由主 `js-eyes` OpenClaw 插件在启动时自动发现，并通过 `skill/<skillId>/<action>` 路由执行
...
**通过 AI Agent：** Agent 调用 `js-eyes` 工具，使用 `action: skills/plan-install` 并在 `args` 中传入技能 ID — 自动下载、校验并生成安装计划；随后执行 `js-eyes skills approve <id>` 与 `js-eyes skills enable <id>` 完成落地。运行中的主插件会通过 `SkillRegistry` + chokidar 在 ~300 ms 内**零重启热加载**该技能。
...
**手动安装：** 从 [js-eyes.com/skills/js-x-ops-skill/](https://js-eyes.com/skills/js-x-ops-skill/js-x-ops-skill-skill.zip) 下载技能 zip，解压到 `skills/js-eyes/skills/js-x-ops-skill/`，执行 `npm install`，随后 `js-eyes skills enable js-x-ops-skill`。运行中的主插件会通过 config watcher 自动热加载；也可显式 `js-eyes skills reload` 或让 Agent 通过 `js-eyes` 调用 `action: skills/reload`。
...
- 保留默认 `skillsDir`，把额外的技能目录（或父目录）加进 `extraSkillDirs`。extras 是**只读**的：被发现并纳入 `js-eyes` 路由，但 js-eyes 不改动它下面的文件、也不跑完整性校验。

对外部 extras 推荐**零重启**路径：`js-eyes skills link /abs/path/to/my-skill` 会去重追加到 `extraSkillDirs` 并在 ~300 ms 内触发运行中插件的 `registry.reload()`；解除用 `js-eyes skills unlink <path>`，强制触发用 `js-eyes skills reload`，Agent 侧可通过 `js-eyes` 调用 `action: skills/reload`。
...
6. 让 Agent 调用 `js-eyes`，`action: browser/get-tabs`
7. 让 Agent 调用 `js-eyes`，`action: skills/discover`
8. 用 `action: skills/plan-install` 安装一个子技能（或用 `js-eyes skills link <path>` 接入外部技能）。主插件会在 ~300 ms 内通过 config watcher 热加载；可用 `action: skills/reload` 确认（或在 gateway 日志里找 `Hot-loaded skill` / `added` 记录）。
...
| `browser/get-tabs` 返回空 | 点击扩展图标，确认地址正确，点击 Connect |
