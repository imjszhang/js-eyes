...
const { getPageProfile, DEFAULT_WS_ENDPOINT, isGoogleTab } = require('./config');
...
function expandBridgeSource(src, baseDir = BRIDGES_DIR) {
  return src.replace(/^[ \t]*\/\/\s*@@include\s+(\S+)\s*$/gm, (_, rel) => {
...
function shouldCloseCreatedTab(target, opts = {}) {
  if (!target || !target._created) return false;
  if (opts.closeCreatedTab === false) return false;
  if (opts.tab != null) return false;
  return true;
}
...
class Session {
  constructor({ opts = {} } = {}) {
    ...
    this._closedCreated = false;
  }

  log(msg) {
    if (this.opts.verbose) process.stderr.write(`[google-session] ${msg}\n`);
  }
  ...
  async resolveTarget() {
    ...
    const createUrl = this.opts.createUrl || targetUrl || 'https://www.google.com/';

    if (this.opts.forceNewTab) {
      return this._openCreated(createUrl);
    }

    const tabs = await this.listTabs();
    const navigateOnReuse = this.opts.navigateOnReuse === true;
    const reuseAnyGoogleTab = this.opts.reuseAnyGoogleTab === true;

    let hit = pickTabMatchingProfile(tabs, this.pageProfile, { targetUrl });
    if (!hit && reuseAnyGoogleTab) {
      hit = tabs.find((t) => isGoogleTab(t)) || null;
    }
  ...
  async openTargetUrl(url) {
    if (!this.target) {
      const err = new Error('openTargetUrl 前必须先 resolveTarget');
      err.code = 'E_NO_TAB';
      throw err;
    }
    await this.bot.openUrl(url, this.target.rawId);
    await this._waitForReady(20000);
    this.target.url = url;
    return this.target;
  }
    ...
    while (Date.now() < deadline) {
      ...
      if (urlChanged && urlMatches && curHref) {
        try {
          ...
          lastErr = (stateResp && stateResp.data && stateResp.data.reason) || 'state_not_ready';
        } catch (err) {
  ...
  async closeCreatedTab() {
    if (!shouldCloseCreatedTab(this.target, this.opts) || this._closedCreated) return false;
    try {
      await this.bot.closeTab(this.target.rawId);
      this._closedCreated = true;
      this.log(`closed created tab ${this.target.id}`);
      return true;
    } catch (err) {
      this.log(`close created tab failed: ${(err && err.message) || err}`);
      return false;
    }
  }

  async close() {
    try { await this.closeCreatedTab(); } catch (_) {}
    if (this._ownsBot) {
