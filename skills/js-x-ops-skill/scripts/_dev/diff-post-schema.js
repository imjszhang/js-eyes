 ...
 *
 * PR 2 验收脚本：对同一 tweet URL 跑「bridge 主路径」与「老 fallback 路径（JS_X_DISABLE_BRIDGE=1）」两套实现，
 * 比对 post 详情字段级一致性。
 *
 * 用法：
 *   node scripts/_dev/diff-post-schema.js <tweet-url> [--server ws://localhost:18080]
 *
 * 退出码：
 *   0 - schema 一致
 *   1 - schema 有 mismatch
...
function summarize(name, response) {
  ...
  if (sample) {
    ...
    if (Array.isArray(sample.replies)) out.sample.repliesCount = sample.replies.length;
    if (Array.isArray(sample.thread)) out.sample.threadCount = sample.thread.length;
  }
  return out;
}

async function runSide(label, urls, opts, useBridge) {
  ...
  try {
    await bot.connect();
    const r = await getPost(bot, urls, { recordingMode: 'off', noCache: true });
    return r;
  } finally {
...
async function main() {
  ...
  if (!url) {
    process.stderr.write('Usage: node scripts/_dev/diff-post-schema.js <tweet-url> [--server ws://...]\n');
    process.exit(2);
  }
  let bridgeR = null, fallbackR = null, bridgeErr = null, fallbackErr = null;
  try { bridgeR = await runSide('bridge', [url], { server }, true); }
  catch (e) { bridgeErr = { message: e.message, code: e.code || null }; }
  try { fallbackR = await runSide('fallback', [url], { server }, false); }
