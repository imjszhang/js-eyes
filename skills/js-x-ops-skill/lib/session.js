...
class Session {
  ...
  log(msg){
    if (this.opts.verbose) process.stderr.write(`[x-session] ${msg}\n`);
  }
  ...
  async resolveTarget(){
    ...
    const reuseAnyXTab = this.opts.reuseAnyXTab === true;
    const createUrl = this.opts.createUrl || targetUrl || null;

    let hit = pickTabMatchingProfile(tabs, this.pageProfile, { targetUrl });
    if (!hit && reuseAnyXTab) {
      hit = tabs.find((t) => /(?:^|\.)(?:x\.com|twitter\.com)\//i.test((t && t.url) || '')
                           || /https?:\/\/(?:www\.)?(?:x|twitter)\.com\//i.test((t && t.url) || '')) || null;
    }
