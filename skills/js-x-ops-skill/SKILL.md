...
- **v3.0.6**：面向第三方复用的 5 个反向增强：
  - `lib/monitor/runCheck.js` 拆出 `runCheckCore({ config, browser, options })`：只做抓 + 去重 + state 读写，**不触发通知**；原 `runCheck` 行为不变（内部在 core 外面套 `dispatch`）
  - `lib/monitor/paths.js` / `state.js`：所有函数新增可选 `{ home }` / `{ stateHome }` 参数，优先级 `显式 > JS_X_MONITOR_HOME env > 默认`；允许调用方（如 moltbook 的 `KolPatrolCollector`）用独立 state 目录，不污染 ops-skill 默认 monitor 目录
  - `lib/monitor/config.js` 导出 `validateConfig(raw)`：内存 config 也能跑 schema 校验（返回 `{ ok, errors, config }`，不抛错）
  - `bridges/common.js::sessionStateCommon`：返回值新增 `username` / `screenName` / `userId` / `displayName` 字段，方便调用方做 whoami（`userId` / `displayName` 当前 settings.json 端点拿不到，保持 null；`name` 字段保留向后兼容，继续 = screen_name）
  - `post` CLI：`node index.js post <id1> <id2> ...` 多 positional 走 `lib/api.js::getPost(browser, tweetInputs, options)` 批量路径；单 URL 仍走 `runTool` 单推文路径保持行为不变
  - 新增 `tests/` 目录（`node --test`），26 个单测覆盖 runCheckCore / validateConfig / stateHome / sessionStateCommon / post CLI 分派
