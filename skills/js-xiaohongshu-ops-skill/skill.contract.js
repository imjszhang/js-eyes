...
const TOOL_DEFINITIONS = [
  ...
  // -------------------- v2.2 搜索域 --------------------
  {
    ...
    description: '小红书搜索（DOM 滚动 + 频道/筛选/联想/相关搜索）。',
    interactive: false,
    destructive: false,
    parameters: {
      type: 'object',
      properties: {
        ...
        sortBy: { type: 'string', description: '排序选项标签（如「最新」「最热」）' },
        contentType: { type: 'string', description: '内容类型选项标签' },
        timeRange: { type: 'string', description: '时间范围选项标签' },
        searchScope: { type: 'string', description: '搜索范围选项标签' },
        extractDetails: { type: 'boolean', description: '是否抽取每条笔记详情（性能开销大，默认 false）' },
      },
      required: ['keyword'],
    },
    execute: makeReadToolExecutor({
