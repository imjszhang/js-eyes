...
function buildMetrics(scrapeResult, durationMs, cacheHit) {
  ...
  return {
    ...
    parsedCommentCount: countComments(comments),
    likeCount: parseOptionalInt(result.like_count),
    commentCount: parseOptionalInt(result.comment_count),
    shareCount: parseOptionalInt(result.share_count),
...
function buildHistoryEntry(runContext, response, options = {}) {
  ...
  return {
    ...
    parsed_comment_count: metrics.parsedCommentCount ?? null,
    image_count: metrics.imageCount ?? null,
    like_count: metrics.likeCount ?? null,
    comment_count: metrics.commentCount ?? null,
    share_count: metrics.shareCount ?? null,
  };
}
...
async function getPost(browser, url, options = {}) {
  ...
  try {
    const result = await scrapeJikePost(browser, url, { runContext });
