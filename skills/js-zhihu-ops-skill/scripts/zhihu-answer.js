...
async function main() {
  ...
  if (!options.url || options.url === '--help' || options.url === '-h') {
    console.log('用法: node index.js answer <url> [--pretty] [--browser-server ws://localhost:18080] [--recording-mode standard] [--debug-recording] [--no-cache]');
    return;
  }
  ...
  try {
    const result = await getAnswer(browser, options.url, {
      ...options,
