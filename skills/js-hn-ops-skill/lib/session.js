...
class Session {
  ...
  log(msg){
    if (this.opts.verbose) process.stderr.write(`[hn-session] ${msg}\n`);
  }
  ...
  async resolveTarget(){
    ...
    const reuseAnyHnTab = this.opts.reuseAnyHnTab === true;
    const createUrl = this.opts.createUrl || targetUrl || null;

    let hit = pickTabMatchingProfile(tabs, this.pageProfile, { targetUrl });
    if (!hit && reuseAnyHnTab) {
      hit = tabs.find((t) => /news\.ycombinator\.com/i.test((t && t.url) || '')) || null;
    }
