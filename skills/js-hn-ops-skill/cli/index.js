...
function buildSessionOpts(commandName, opts, extra = {}) {
  return {
    ...
    reuseAnyHnTab: extra.reuseAnyHnTab !== false,
...
const XHR_LOG_FN_SRC = `function(args){
  args = args || {};
  const filterRaw = args.filter || 'firebaseio\\.com|algolia\\.com|news\\.ycombinator\\.com';
  let filterRegex;
  try { filterRegex = new RegExp(filterRaw); } catch(_) { filterRegex = /firebaseio\\.com|algolia\\.com|news\\.ycombinator\\.com/; }
...
if (require.main === module) {
  main(process.argv.slice(2)).then((code) => process.exit(code || 0)).catch((err) => {
    process.stderr.write(`ERROR: ${err.message}\n`);
    if (process.env.JS_HN_DEBUG) process.stderr.write((err.stack || '') + '\n');
    process.exit(1);
  });
}
