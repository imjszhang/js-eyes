...
- **First-class page interaction vs raw eval.** Wire actions `click`, `fill`, `scroll`, and `wait_for` run fixed, extension-owned `scripting.executeScript` functions with structured arguments. They do **not** accept caller-supplied JavaScript and do **not** require `allowRawEval`. Prefer these (and MCP safe-profile `browser_click` / `browser_fill` / `browser_scroll` / `browser_wait_for`) for ordinary UI automation. Keep `execute_script` for cases that truly need arbitrary code, behind the full MCP profile and host policy.
...
- **L4b — Lightweight Taint (`TaintRegistry`).** Every cookie value returned by `getCookies*` is tagged with an 8-byte canary (`__canary: "jse-c-<hex>"`) and registered. Subsequent sink parameters (`openUrl`, `uploadFileToTab`, `executeScript`, `injectCss`, `setCookies`) are scanned for the canary or common-encoded cookie-value variants. Hits are soft-blocked and audited as `reason: 'taint-hit'`. `syncCookies` is a controlled server-side channel: values stay in server-core memory and are not written to audit, MCP, or CLI output.
...
in-memory cookie sync, the single OpenClaw router, and the host-neutral Skill
