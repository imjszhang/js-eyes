...
> **2.10.0 note**: This document covers the runtime security posture (network
...
> switch / verify) see the [Security Posture table in `README.md`](./README.md#security-posture-2100).
...
- **Safe ZIP extraction.** `packages/skill-install/zip-extract.js` (`@js-eyes/skill-install/zip-extract`) replaces `execSync unzip` / PowerShell `Expand-Archive` with an in-process ZIP reader that rejects Zip Slip, symlinks, and oversized entries (`maxFileSize`, `maxTotalSize`, `maxEntries`).
...
- **Loopback binding.** The server refuses to bind to a non-loopback host unless `security.allowRemoteBind=true`. When bound to a public address, a warning is logged and audited. A LAN browser on another machine also needs `serverHost` set to a non-loopback address (typically `0.0.0.0`) and must paste `server.token` in the extension; Native Messaging cannot sync the token across hosts. See the README sections [Remote browser on the LAN](./README.md#remote-browser-on-the-lan) and [Headless and no-display hosts](./README.md#headless-and-no-display-hosts).
- **Optional CDP / BiDi connectors.** These are outbound debug-protocol attachments, default `enabled=false` and `loopbackOnly=true`. They are an explicit opt-in, not a replacement for the extension. Non-loopback endpoints also require `security.allowRemoteBind`. Connect/disconnect is audited as `browser.connector.connect` / `browser.connector.disconnect`. Policy, taint, and egress still run before dispatch. `execute_script` on these connectors still requires `security.allowRawEval`. Raw CDP/BiDi is never exposed to agents.
...
- **Sensitive action set.** `protocol.SENSITIVE_TOOL_NAMES` currently contains `browser/execute-script`, `browser/get-cookies`, `browser/get-cookies-by-domain`, `browser/sync-cookies`, `browser/upload-file`, `browser/inject-css`, `browser/list-downloads`, `browser/wait-download`, and `skills/plan-install`. `browser/set-cookies` is a connector primitive used by server-side sync and the SDK; it is not an MCP or OpenClaw tool. Additional actions can be added via `security.toolPolicies`. OpenClaw `toolPolicies` default `browser/list-downloads` and `browser/wait-download` to `confirm`.
...
- **First-class page interaction vs raw eval.** Wire actions `click`, `fill`, `scroll`, `wait_for`, `get_page_state`, `send_keys`, `navigate_history`, and `select_option` run fixed, connector-owned in-page functions with structured arguments. They do **not** accept caller-supplied JavaScript and do **not** require `allowRawEval`. Prefer `page.state` refs, then click/fill (and MCP safe-profile `browser_page_state` / `browser_click` / `browser_fill` / `browser_send_keys` / `browser_select`). Keep `execute_script` for cases that truly need arbitrary code, behind the full MCP profile and host policy.
- **Chrome `downloads` permission.** The Chrome extension now requests the store-facing `downloads` permission so `downloads.list` / `downloads.wait` can return metadata only (`id`, `basename`, `state`, `bytes`, `mime`, `urlHost`). Responses never include file bodies, full local paths, or base64. CDP launch uses `Browser.setDownloadBehavior` into `~/.js-eyes/downloads`.
- **Secret-ref fill.** `fill` may take `secretRef` instead of `value`. The name is resolved only in server-core / the skill host from `~/.js-eyes/secrets/<name>` (`0600`). Audit, MCP, CLI, and Skill recordings log the name, never the plaintext. `js-eyes secrets list` prints names only.
- **Wait for user is not a Policy confirm.** `page.waitForUser` writes `runtime/pending-user/<id>.json` (`0600`) and waits for the extension popup Continue button or `js-eyes browser resume <id>` over the existing authenticated channel. It does not add interactive PolicyEngine `confirm` dialogs (Non-Goals still hold).
...
- **L4a — Same-Origin Task (`TaskOriginTracker`).** Merges a scope set from four sources: user messages (URLs / bare domains), `skill.contract.runtime.platforms`, the current active tab URL, and links found on HTML that the agent has already fetched. `getCookies` / `getCookiesByDomain` / `setCookies` / `syncCookies` / `executeScript` / `injectCss` / `uploadFileToTab` are evaluated against this scope.
- **L4c — Sensitive cookie domains.** `security.sensitiveCookieDomains` (hot-reloadable; default includes `google.com`, `apple.com`, `bank`, and similar hosts) is now enforced on cookie read/write/sync. A matching host is soft-blocked by default and denied in `strict`.
- **L4b — Lightweight Taint (`TaintRegistry`).** Every cookie value returned by `getCookies*` is tagged with an 8-byte canary (`__canary: "jse-c-<hex>"`) and registered. Subsequent sink parameters (`openUrl`, `uploadFileToTab`, `executeScript`, `injectCss`, `setCookies`, and resolved `fill` values) are scanned for the canary or common-encoded cookie-value variants. Hits are soft-blocked and audited as `reason: 'taint-hit'`. `syncCookies` is a controlled server-side channel: values stay in server-core memory and are not written to audit, MCP, or CLI output. `secretRef` names may appear in logs; secret values must not.
...
- **L6 — Rule Engine Location.** The engine lives in `@js-eyes/policy` so that skills, the client SDK, and the OpenClaw plugin share one implementation. `@js-eyes/client-sdk` re-exports selected policy symbols from its main entry for convenience; new code should depend on `@js-eyes/policy` directly. `@js-eyes/server-core/ws-handler` instantiates the same engine to cover raw WebSocket callers (external agents that bypass `client-sdk`).
...
*Last updated: 2026-09-21 — covers the policy engine, Native Messaging host,
host-synced `allowRawEval`, security hot reload, enforced `sensitiveCookieDomains`,
in-memory cookie sync, first-class page state/refs, secret-ref fill, download
metadata, wait-for-user, the single OpenClaw router, and the host-neutral Skill
Runtime V2 trust boundary.*
